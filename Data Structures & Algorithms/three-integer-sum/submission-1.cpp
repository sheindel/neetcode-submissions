...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        for (auto it = nums.begin(); it < nums.end()-2; it++) {
            if (*it > 0) {
                // we can stop early, if the lowers number is already positive, everything
                // after is positive too.
                break;
            }
            if (it > nums.begin() && *it == *(it - 1)) {
                continue; // we already evaluated this value, so we don't need to try again
            }
